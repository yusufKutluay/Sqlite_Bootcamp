package com.yusufkutluay.sqlite_bootcamp.uix.views.detailpage
...
import com.yusufkutluay.sqlite_bootcamp.uix.views.notespage.CurrentDate
import com.yusufkutluay.sqlite_bootcamp.uix.views.notespage.ModalBottomSheetScreen
import com.yusufkutluay.sqlite_bootcamp.uix.views.notespage.isDarkColor
import kotlinx.coroutines.launch


class DetailPage(var detailPageViewModel: DetailPageViewModel,var notesDetail:Notes) : Screen{

    @OptIn(ExperimentalMaterial3Api::class)
    @RequiresApi(Build.VERSION_CODES.R)
    @Composable
    override fun Content() {
        val title = remember { mutableStateOf(notesDetail.title) }  // room
        val notes = remember { mutableStateOf(notesDetail.notes) }  // room
        val colorPage = remember { mutableStateOf(notesDetail.colorPage) } // room
        val textSize = remember { mutableStateOf(notesDetail.textSize) } // room
        val textColor = remember { mutableStateOf(notesDetail.textColor) } // room
        val menuMore = remember { mutableStateOf(false) }
        val colorsHex = detailPageViewModel.colorlist.value
        val todoOk = remember { mutableStateOf(notesDetail.todoOk) }
        val sheetState = rememberModalBottomSheetState()
        val showBottomSheet = remember { mutableStateOf(false) }
        val date = remember { mutableStateOf(notesDetail.currentDate) } // room
        val selectedNotes = remember { mutableStateOf<Int?>(null) }
        ...
        DisposableEffect(colorPage.value) {
            ...
            activity.window.navigationBarColor =
                Color(colorPage.value.toColorInt()).toArgb() // gezinme çubuğu
            ...
            if (isDarkColor(colorPage.value)) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    ...
                    activity.window.decorView.systemUiVisibility =
                        View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR // İkonlar siyah olur
                }
            }
        ...
        Scaffold { paddingValues ->
            ...
            if (showAlertDialog.value){
                AlertDialog(
                    onDismissRequest = { showAlertDialog.value },
                    text = { Text(text = "Bu not silinsin mi")},
                    confirmButton = {
                        OutlinedButton(onClick = {
                            detailPageViewModel.delete(selectedNotes.value!!)
                    ...
                    dismissButton = {
                        OutlinedButton(onClick = { showAlertDialog.value = false }) {
                            Text(text = "İptal")
                        }
                    }
                )
            ...
            Column(
                ...
                Row(
                     ...
                     */
                    IconButton(onClick = {
                        detailPageViewModel.update(notesDetail.id,title.value,notes.value,colorPage.value,textColor.value,textSize.value, date.value,todoOk.value)
                        navigator.popUntilRoot()
                    }, Modifier) {
                        Icon(
                    ...
                    Box(modifier = Modifier.size(50.dp).weight(10f)) {
                        Box(
                            ...
                            IconButton(onClick = { menuMore.value = !menuMore.value }) {
                                ...
                                ) // Icons.Filled.MoreVert menu iconu alınır
                            }
                            DropdownMenu(
                                ...
                                DropdownMenuItem(
                                    text = { Text(text = "Sil") },
                                    onClick = {
                                        selectedNotes.value = notesDetail.id
